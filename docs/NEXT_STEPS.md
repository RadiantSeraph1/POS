...
55. boots an Electron + React warehouse shell on top of the receiving queue and transfer dashboard projection layer
56. supports embedded or external backend mode in the warehouse shell
57. submits transfer receipt quantities from the warehouse shell through the protected backend
58. projects `STOCK_TRANSFER_REJECTED` and `STOCK_TRANSFER_CANCELLED` through backend replay and reconciliation
59. supports selected transfer discrepancy detail in the warehouse shell
60. supports reject/cancel lifecycle actions for eligible transfers in the warehouse shell
61. supports approval quantity editing and `STOCK_TRANSFER_APPROVED` submission from the warehouse shell
62. supports dispatch quantity editing and `STOCK_TRANSFER_DISPATCHED` submission from the warehouse shell
...
1. Expand the cashier shell from summary-and-filter recovery into richer recent-sale inspection, shift-close reporting, and stronger root-cause tooling for queue failures.
